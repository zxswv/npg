...
・カレンダーからキャンセルした予約がキャンセル済みに表示されない(キャンセルした予約が存在ごと消される)
```
...
・API の必要な項目、データのみの最適化
・データベースの予約データ追加の仕組みを(現：/api/timeline/route.tsAPI 完結型 → 未来：/prisma/seed.ts でデータを削除せずに新しいデータを作成する)に変更する。
・ユーザー管理  
・権限管理  
・授業登録画面  
・掲示板  
